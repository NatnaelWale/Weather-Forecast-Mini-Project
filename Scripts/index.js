...
const selectedCityForecastData = document.getElementById(
...
function populateSelectname() {
  ...
  citySelect.addEventListener("change", () =>
    populateCitiesData(citySelect.value)
  );
    ...
    .then((data) => {
      ...
      fetch(newApiUrl)
        ...
        .then((data) => {
          ...
          ["Time/Date", "Temprature", "Wind", "Forecast", "Icon"].forEach(
            (text) => {
          ...
          for (let forecast of data.properties.periods) {
            ...
            row.appendChild(
              createCell(
                `Temprature ${forecast.temperature} ${forecast.temperatureUnit}`
              )
            );
            row.appendChild(
              createCell(
                `Winds ${forecast.windDirection} ${forecast.windSpeed}`
              )
            );
